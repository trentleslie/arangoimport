...
This script includes proper timeout handling and retries for working with large databases.
"""
...
def main():
    ...
    # Create a client connection with a longer timeout
    client = ArangoClient(hosts=f"http://{host}:{port}", request_timeout=300)
    ...
    try:
        ...
        # Add node label index (most critical for performance)
        node_label_index = {
            "type": "persistent",
            "fields": ["labels"],
        ...
        add_index_with_retry(db.collection("Nodes"), node_label_index)
